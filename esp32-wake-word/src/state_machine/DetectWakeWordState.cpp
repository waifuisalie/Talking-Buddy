...
bool DetectWakeWordState::run()
{
    ...
    if (output > 0.95)
    {
        ...
        {
            m_number_of_detections = 0;
            // Wake word "Marvin" detected!
            Serial.printf("🎙️ MARVIN_DETECTED (confidence: %.2f)\n", output);
            Serial.println("WAKE_WORD_DETECTED");
            Serial.flush();

            // Stay in DetectWakeWordState forever
            // The trigger will be handled by Python via serial message
