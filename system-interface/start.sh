...
if ! ollama list | grep -qE "gemma3:1b|gemma3-ptbr|qwen2\.5.*-ptbr|llama3\.2.*-ptbr|qwen3\.5.*-ptbr|qwen3\.5:0\.8b"; then
    echo -e "${YELLOW}⚠️  Nenhum modelo encontrado. Baixando qwen3.5:0.8b...${NC}"
    ollama pull qwen3.5:0.8b
fi

# Count personality models
PERSONALITY_COUNT=$(ollama list | grep -cE "(gemma3|qwen2\.5|llama3\.2|qwen3\.5).*-ptbr-" 2>/dev/null || echo "0")
...
OLLAMA_WARMUP_MODEL="${OLLAMA_MODEL:-qwen3.5:0.8b}"
